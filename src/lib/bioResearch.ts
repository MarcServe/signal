...
/** Calls POST /api/artist-bio-research (Perplexity if configured, else Wikipedia). */
export async function fetchArtistBioFromWeb(query: string): Promise<BioResearchResult> {
  ...
  try {
    const res = await fetch(apiUrl('/artist-bio-research'), {
      method: 'POST',
      headers: {
...
export async function polishArtistBioDraft(draft: string, displayName?: string): Promise<BioPolishResult> {
  ...
  if (!session) return { ok: false, error: 'Session expired. Sign in again.' }
  try {
    const res = await fetch(apiUrl('/artist-bio-polish'), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${session.access_token}`,
      },
      body: JSON.stringify({ draft: d, display_name: displayName?.trim() || undefined }),
    })
