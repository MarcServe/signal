...
- Build command: `npm run build` (runs **`vite build`** to `dist/`, then **`vite build --config vite.vercel.config.ts`** so `vite-plugin-vercel` can copy `dist/` into `.vercel/output/static` and bundle `/api/*`). **Output directory: `.vercel/output`** (see `vercel.json`). **All `/api/*` URLs are handled by one serverless function** (`api/[...path].ts`) → `api/_routes/*` (Hobby **12 functions** limit). Skipping the second Vite step leaves **no static files** → root URL **404**.
